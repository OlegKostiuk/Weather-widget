		...
		try {
			URL url = new URL(
					"http://api.worldweatheronline.com/free/v1/weather.ashx?q="
							+ location
							+ "&format=xml&fx=no&key=4ysvy8mc459skxg64a9zrbwk");
			Document doc;
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (child instanceof Element) {
					if (child.getNodeName().equals("request")) {
						...
						for (int j = 0; j < childNodes.getLength(); j++) {
							if (childNodes.item(j).getNodeName()
									.equals("query")) {
								currentWeather.setLocation(childNodes.item(j)
										.getTextContent());
							}
						}
					...
					if (child.getNodeName().equals("current_condition")) {
						...
						for (int j = 0; j < childNodes.getLength(); j++) {
							String textContent = childNodes.item(j)
									.getTextContent();
			...
			System.err.println("Exception:" + e.toString() + " Internet connection or API error");
		}
